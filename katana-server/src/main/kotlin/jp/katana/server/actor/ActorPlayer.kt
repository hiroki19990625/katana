...
class ActorPlayer(override val address: InetSocketAddress, private val server: Server) : IActorPlayer {
    ...
    init {
        ...
        data.setData(ActorDataIds.DATA_NAME_TAG,
            StringActorData(displayName)
        )
        data.setData(ActorDataIds.DATA_LEAD_HOLDER_EID, LongActorData(-1))
        data.setData(ActorDataIds.DATA_SCALE, FloatActorData(1f))
        data.setData(ActorDataIds.DATA_BOUNDING_BOX_WIDTH,
            FloatActorData(width.toFloat())
        )
        data.setData(ActorDataIds.DATA_BOUNDING_BOX_HEIGHT,
            FloatActorData(height.toFloat())
        )
